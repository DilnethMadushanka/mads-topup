...
import { 
  ...
  Clipboard, Plus, Minus, ChevronUp, ChevronDown, HelpCircle, Shield
} from 'lucide-react';
      ...
      {completedOrder ? (
        ...
        <>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-5 sm:p-7 mb-6">
            ...
            {ignVerified && (
              <div className="mt-3 p-3 bg-emerald-50 border border-emerald-300 rounded-xl flex items-center justify-between text-xs text-emerald-800 font-semibold animate-in fade-in">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                  <span>Verified IGN: <strong>{ign}</strong></span>
                </div>
                <button 
                  onClick={() => savePlayerId(selectedGame.id, selectedGame.name, playerId, ign)}
                  className="text-[11px] bg-emerald-600 text-white px-2.5 py-1 rounded-md font-bold hover:bg-emerald-700 transition-colors flex items-center gap-1 cursor-pointer"
                >

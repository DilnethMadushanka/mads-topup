  ...
  return (
    <section id="game-catalog" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <div className="flex items-center gap-2 text-red-600 text-xs font-black uppercase tracking-wider mb-2">
      ...
      <div className="md:hidden mb-8">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-semibold focus:outline-none focus:border-red-500 shadow-sm"
          />
        </div>
      </div>
      ...
      {filteredGames.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border border-slate-200 shadow-sm">
          ...
          <button 
            ...
            className="mt-4 px-5 py-2.5 text-xs font-black bg-red-100 text-red-600 rounded-xl"
          >
            Reset Filters
          </button>
        </div>
          ...
          {filteredGames.map((game) => {
            ...
            return (
              <div
                ...
                className="group relative bg-white rounded-3xl border border-slate-200/80 card-shadow-premium card-shadow-hover overflow-hidden cursor-pointer flex flex-col justify-between"
              >
                {/* Banner Artwork */}
                <div className="relative h-48 overflow-hidden bg-slate-950">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="px-3 py-1 rounded-full bg-red-600 text-white text-[10px] font-black uppercase tracking-wider shadow-md">
                      {game.badge}
                    </span>
                  </div>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {game.packages.slice(0, 3).map(p => (
                        <span 
                          key={p.id} 
                          className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 text-xs font-bold border border-slate-200/60"
                        >
                          {p.name}
                        </span>
                      ))}
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <div className="px-5 py-2.5 rounded-xl bg-red-600 text-white font-black text-xs group-hover:bg-red-700 transition-colors flex items-center gap-1 shadow-md shadow-red-600/20">
                      <span>Top Up</span>

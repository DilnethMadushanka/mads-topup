...
export const MobileBottomNav = () => {
  const { setIsUserProfileOpen, setIsAdminOpen, orders, openCatalog, closeCatalog, isGameCatalogOpen } = useApp();
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-t border-slate-200 px-3 py-2 flex items-center justify-around shadow-2xl">
      <button 
        ...
        className={`flex flex-col items-center gap-1 font-bold text-[11px] cursor-pointer ${!isGameCatalogOpen ? 'text-[#00B4D8]' : 'text-slate-600'}`}
      >

      ...
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-slate-900 text-white p-5 flex items-center justify-between border-b border-slate-800">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-red-600 to-rose-500 flex items-center justify-center font-bold text-xl shadow-md text-white">
            ...
            <div>
              <h2 className="text-xl font-black font-heading tracking-tight flex items-center gap-2">
                ...
                <span className="text-[10px] bg-red-600 px-2 py-0.5 rounded-full font-extrabold uppercase">
                  Moongold 24/7
                </span>
              </h2>
              <p className="text-xs text-slate-300">Fast & Automated Delivery to Account</p>
            </div>
          </div>
        ...
        <div className="bg-slate-100 px-6 py-3 border-b border-slate-200 flex items-center justify-between text-xs font-bold">
          <div className={`flex items-center gap-1.5 ${step >= 1 ? 'text-red-600' : 'text-slate-400'}`}>
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${step >= 1 ? 'bg-red-600 text-white' : 'bg-slate-300 text-slate-600'}`}>1</span>
            <span>Account ID</span>
          </div>
          <ChevronRight className="w-4 h-4 text-slate-300" />
          <div className={`flex items-center gap-1.5 ${step >= 2 ? 'text-red-600' : 'text-slate-400'}`}>
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${step >= 2 ? 'bg-red-600 text-white' : 'bg-slate-300 text-slate-600'}`}>2</span>
            <span>Package</span>
          </div>
          <ChevronRight className="w-4 h-4 text-slate-300" />
          <div className={`flex items-center gap-1.5 ${step >= 3 ? 'text-red-600' : 'text-slate-400'}`}>
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${step >= 3 ? 'bg-red-600 text-white' : 'bg-slate-300 text-slate-600'}`}>3</span>
            <span>Payment</span>
          </div>
        </div>
            ...
            <>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {userProfile.savedIds.filter(s => s.gameId === selectedGame.id).length > 0 && (
                    <span className="text-[11px] text-slate-500 font-semibold">Saved IDs Available</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 pt-1">
                  <div className={selectedGame.requiresServer ? 'sm:col-span-7' : 'sm:col-span-8'}>
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm font-semibold focus:outline-none focus:border-red-500 shadow-sm"
                    />
                  </div>
                  ...
                  <div className={selectedGame.requiresServer ? 'sm:col-span-12' : 'sm:col-span-4'}>
                    <button
                      ...
                      className="w-full h-full min-h-[42px] px-3 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold hover:bg-slate-800 transition-colors flex items-center justify-center gap-1.5 shadow-sm"
                    >
                      {isVerifyingIgn ? (
              ...
              <div className="space-y-3">
                <label className="text-xs font-extrabold text-slate-700 uppercase tracking-wider flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-red-600 font-bold">100% Guaranteed Delivery</span>
                </label>
                    ...
                    return (
                      <div
                        ...
                        className={`relative p-3.5 rounded-2xl border-2 cursor-pointer transition-all flex flex-col justify-between ${
                          isSelected
                            ? 'bg-red-50/50 border-red-600 shadow-md shadow-red-600/10'
                            : 'bg-white border-slate-200 hover:border-slate-300'
                        }`}
                      >
                        {pkg.isPopular && (
                          <span className="absolute -top-2.5 right-2 px-2 py-0.5 rounded-full bg-red-600 text-white text-[9px] font-black uppercase tracking-wider shadow">
                            POPULAR
                          </span>
                        )}
                        ...
                        <div>
                          ...
                          {pkg.bonus && (
                            <span className="text-[10px] text-red-600 font-bold bg-red-100 px-1.5 py-0.5 rounded inline-block">
                              {pkg.bonus}
                            </span>
                          )}
                        </div>
                  ...
                  {selectedPayment.accountDetails && (
                    <div className="p-4 bg-slate-100 rounded-2xl border border-slate-200 text-xs space-y-2">
                      ...
                      {selectedPayment.id === 'bank' && (
                        <div className="bg-white p-3 rounded-xl border border-slate-200 space-y-1 font-mono text-[11px] text-slate-700">
                      ...
                      {selectedPayment.id === 'ezcash' && (
                        <div className="bg-white p-3 rounded-xl border border-slate-200 font-mono text-[11px] text-slate-700">
                          <div>eZ Cash Number: <strong className="text-red-600 font-extrabold">{selectedPayment.accountDetails.number}</strong></div>
                        </div>
                      )}
            ...
            <div className="text-center py-6 space-y-5 animate-in zoom-in-95">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 text-left max-w-md mx-auto space-y-2 text-xs">
                ...
                <div className="flex justify-between border-b border-slate-200 pb-2">
                  ...
                  <span className="font-mono font-bold text-red-600">{completedOrder.moongoldRef}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200 pb-2">
                ...
                <div className="flex justify-between pt-1 font-bold text-sm">
                  ...
                  <span className="text-red-600">{formatPrice(completedOrder.priceLkr)}</span>
                </div>
              </div>
          ...
          <div className="bg-slate-50 p-4 px-6 border-t border-slate-200 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {step === 3 && (
                <button
                  onClick={() => setStep(2)}
                  className="px-4 py-2.5 text-xs font-bold text-slate-600 hover:text-slate-900 bg-white border border-slate-300 rounded-xl"
                >
                  Back
                </button>
              )}
              ...
              {step < 3 ? (
                <button
                  ...
                  className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-extrabold text-sm rounded-xl transition-all shadow-md shadow-red-600/30 flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="px-6 py-3 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-black text-sm rounded-xl transition-all shadow-lg shadow-red-600/40 flex items-center gap-2"
                >
                  {isSubmitting ? (
                    <>

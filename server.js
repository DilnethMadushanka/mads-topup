...
// ─────────────────────────────────────────────────────────────────────────
// Free Fire Player Info API  /api/player-info?uid=&region=
// Uses RapidAPI (check-id-game3) for live lookups.
// Falls back to deterministic mock data if the API call fails.
// ─────────────────────────────────────────────────────────────────────────
function buildMockPlayerData(uid, region) {
  const names  = ['🔥 S L _ S L A Y E R 🔥', '⚡ D A R K_K I N G ⚡', '👑 ALPHA_BEAST 👑', '💀 SKULL_X_99 💀'];
  const guilds = ['DARK ELITE', 'COBRA SQUAD', 'FIRE LORDS', 'SHADOW CREW'];
  const pets   = [
    { name: 'Ottero', icon: '🦦', id: 'PET-1104' },
    { name: 'Falco',  icon: '🦅', id: 'PET-1007' },
    { name: 'Poring', icon: '🐸', id: 'PET-1202' },
  ];
  const seed = parseInt(uid.slice(-4), 10) || 1;
  ...
  return {
    ...
    accountInfo: {
      ign: pick(names),
      level: 60 + (seed % 40),
      primeLevel: 10 + (seed % 10),
      totalLikes: 50_000 + seed * 13,
      createdAt: '2019-08-12',
      lastLogin: new Date().toISOString().split('T')[0],
      badges: 20 + (seed % 30),
      creditScore: 100,
      signature: 'Top Free Fire player · Sri Lanka 🇱🇰',
      avatar: `https://ui-avatars.com/api/?name=FF+${uid.slice(-3)}&background=cc040a&color=fff&size=128&bold=true&rounded=true`,
    },
    pet: { ...pet, level: 5 + (seed % 5), exp: 1000 + seed * 7, maxExp: 3000 },
    outfit: [
      { name: 'Demon Slayer Helmet', slot: 'Head',   icon: '🪖' },
...
// Map the raw RapidAPI response to our unified card schema
function mapRapidApiResponse(raw, uid, region) {
  // check-id-game3 returns various shapes — handle both known formats
  const d = raw?.data || raw || {};

  const ign         = d.nickname || d.name || d.username || d.playerName || d.accountName || d.account_name || `Player_${uid.slice(-5)}`;
  const level       = parseInt(d.level || d.accountLevel || d.account_level || d.rank || 0) || 1;
  const primeLevel  = parseInt(d.primeLevel || d.prime_level || d.seasonId || d.season || 0) || 1;
  const totalLikes  = parseInt(d.liked || d.likes || d.totalLikes || d.total_likes || 0) || 0;
  const createdAt   = d.createAt || d.created_at || d.accountCreatedAt || '—';
  const lastLogin   = d.lastLoginAt || d.last_login || d.loginAt || new Date().toISOString().split('T')[0];
  const badges      = parseInt(d.badges || d.badgeCount || d.badge_count || 0) || 0;
  const creditScore = parseInt(d.creditScore || d.credit_score || 100) || 100;
  const signature   = d.signature || d.bio || d.title || '';
  const avatar      = d.avatarImage || d.avatar || d.icon || d.profileUrl ||
    `https://ui-avatars.com/api/?name=${encodeURIComponent(ign.slice(0,10))}&background=cc040a&color=fff&size=128&bold=true&rounded=true`;

  // Pet
  const petRaw  = d.pet || d.petInfo || {};
  const petName = petRaw.name || petRaw.petName || 'Unknown Pet';
  const petLevel= parseInt(petRaw.level || 1);
  const petId   = petRaw.id || petRaw.petId || 'PET-0000';
  const petExp  = parseInt(petRaw.exp || petRaw.currentExp || 0);
  const petMax  = parseInt(petRaw.maxExp || petRaw.max_exp || 3000) || 3000;

  // Guild
  const guildRaw     = d.clan || d.guild || d.guildInfo || {};
  const guildName    = guildRaw.name || guildRaw.clanName || guildRaw.guildName || '—';
  const guildId      = guildRaw.id || guildRaw.clanId || guildRaw.guildId || '—';
  const guildLevel   = parseInt(guildRaw.level || 1);
  const guildMembers = parseInt(guildRaw.memberNum || guildRaw.members || guildRaw.member_count || 0);
  const leaderName   = guildRaw.leaderName || guildRaw.leader_name || guildRaw.ownerName || '—';
  const leaderId     = guildRaw.leaderId || guildRaw.leader_id || guildRaw.ownerId || '—';

  return {
    uid, region,
    accountInfo: { ign, level, primeLevel, totalLikes, createdAt, lastLogin, badges, creditScore, signature, avatar },
    pet: { name: petName, level: petLevel, id: petId, exp: petExp, maxExp: petMax, icon: '🐾' },
    outfit: [
      { name: d.headEquip  || 'Head Item',   slot: 'Head',   icon: '🪖' },
      { name: d.topEquip   || 'Top Item',    slot: 'Top',    icon: '👕' },
      { name: d.bottomEquip|| 'Bottom Item', slot: 'Bottom', icon: '👖' },
      { name: d.shoeEquip  || 'Shoes',       slot: 'Shoes',  icon: '👟' },
    ],
    guild: { name: guildName, id: String(guildId), level: guildLevel, members: guildMembers, leaderName, leaderId: String(leaderId) },
    _source: 'live',
  };
}

app.get('/api/player-info', rateLimiter(30, 60000), async (req, res) => {
  try {
    const uid    = sanitizeString(String(req.query.uid    || ''), 20);
    const region = sanitizeString(String(req.query.region || 'SG'), 10).toUpperCase();

    if (!uid || !/^\d{5,15}$/.test(uid)) {
      return res.status(400).json({ error: 'Invalid UID. Must be 5–15 digits.' });
    }

    const RAPID_KEY = process.env.RAPIDAPI_KEY || process.env.VITE_RAPIDAPI_KEY || '';

    // ── Try real RapidAPI endpoints in order ──────────────────────────────
    if (RAPID_KEY) {
      const regionLower = region.toLowerCase();

      // Candidate endpoint list (tried in order, first success wins)
      const candidates = [
        // check-id-game3 — the API shown in the RapidAPI screenshot
        {
          url: `https://check-id-game3.p.rapidapi.com/game/free-fire?id=${encodeURIComponent(uid)}&region=${encodeURIComponent(regionLower)}`,
          host: 'check-id-game3.p.rapidapi.com',
        },
        // Alternate path without region (some endpoints omit it)
        {
          url: `https://check-id-game3.p.rapidapi.com/game/free-fire?id=${encodeURIComponent(uid)}`,
          host: 'check-id-game3.p.rapidapi.com',
        },
        // free-fire-info fallback
        {
          url: `https://free-fire-info.p.rapidapi.com/player?uid=${encodeURIComponent(uid)}&region=${encodeURIComponent(region)}`,
          host: 'free-fire-info.p.rapidapi.com',
        },
      ];

      for (const { url, host } of candidates) {
        try {
          ...
          if (apiRes.ok) {
            ...
            // Verify we got actual player data (not an error body)
            const hasData = raw && (
              raw.nickname || raw.name || raw.username || raw.playerName ||
              (raw.data && (raw.data.nickname || raw.data.name || raw.data.username))
            );
            if (hasData) {
              console.log(`[FF API] Live data from ${host}`);
              return res.json(mapRapidApiResponse(raw, uid, region));
            }
          }
        } catch (e) {
          console.warn(`[FF API] ${host} failed:`, e.message);
        }
      }
    }

    // ── Fallback: deterministic mock ──────────────────────────────────────
    console.log('[FF API] Using mock fallback for UID:', uid);
    return res.json(buildMockPlayerData(uid, region));
